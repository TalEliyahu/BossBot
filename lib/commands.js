        ...
        if (msg.chat.type === 'supergroup') {
            ...
            if (this.commonFunctions.messageSenderIsAdmin(admins, msg)) {
                ...
                if (!enoughtRights) {
                    ...
                    await this.collections.mongoWhiteList.insertOne({ groupId: msg.chat.id, links: [] });
    ...
    *Available commands:*
    ...
    async whiteList(msg, linksString) {
        ...
        if (group) {
            ...
            const groupWhiteList = await mongoWhiteList.findOne({ groupId });
            const currentLinks = groupWhiteList.links;
            ...
            if (links.length === 0) {
                ...
                groupWhiteList.links = Array.from(new Set(groupWhiteList.links.concat(linksToSave)));
                await mongoWhiteList.save(groupWhiteList);
    ...
    async unWhiteList(msg, linksString) {
        ...
        if (group) {
            ...
            if (links.length === 1 && links[0] === '-') {
                groupWhiteList.links = [];
                await mongoWhiteList.save(groupWhiteList);

                this.bot.sendMessage(msg.from.id, 'Whitelist is cleared');
            } else if (links.length > 0) {
                const message = links.map(l =>
                    groupWhiteList.links.includes(l)
                        ? `Deleted: ${l}`
                        : `Not in whitelist: ${l}`
                ).join('\n');

                groupWhiteList.links = groupWhiteList.links.filter(l => !links.includes(l));
                await mongoWhiteList.save(groupWhiteList);
                this.bot.sendMessage(msg.from.id, message);
            }
        } else {
