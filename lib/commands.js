...
class Commands {
   ...
   */
    ...
    *Available commands:*
    ...
    Show this message
    /blacklist 
    To get a list of all blacklisted words
    /blacklist %word% 
    ...
    async actionHandler(groupId,action){
        if(action=='*resetBlacklist'){
          this.log(this.actionTypes.blacklistWordRemove, msg)
    ...
    async blacklistCommand(msg, match) {
        if (msg.chat.type !== 'private') {
            // unless it's a private chat we show list of blacklist messages
            this.showBlackList(msg);
            return;
        }
        if(!match[2]){
            this.bot.sendMessage(msg.chat.id,"Use /blacklist <word> to blacklist the word");
            return;
        }
        const word = match[2];
        const group = await this.getGroupThatUserCurrentlyConfigures(msg);
        if (group) {
            ...
            await mongoBlacklist.updateOne({ groupId : group.id} , {$push : {words : word.toLowerCase()}} , { upsert: true });
