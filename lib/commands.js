    ...
    async configCommand(msg) {
        ...
        if (msg.chat.type === 'supergroup') {
            ...
            if (this.commonFunctions.messageSenderIsAdmin(admins, msg)) {
                ...
                console.log(cfg.allowAdminsToConfigure);
                console.log(admins.find(x=>x.status == 'creator').user.id != msg.from.id);
                if(!cfg.allowAdminsToConfigure && (admins.find(x=>x.status == 'creator').user.id != msg.from.id)){
        ...
        return {
            inline_keyboard: [
                [{
                    ...
                    callback_data: `${groupConfig.groupId}#allowAdminsToConfigure`
                }]
            ]
        };
    }
