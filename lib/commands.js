    ...
    async configCommand(msg) {
        ...
        if (msg.chat.type === 'supergroup') {
            ...
            if (this.commonFunctions.messageSenderIsAdmin(admins, msg)) {
                ...
                const allowedAdmins = []
                const allowedInDb = await this.collections.mongoAllowedAdmins.findOne({ groupId: group.id });
                if (allowedInDb) {
                    allowedAdmins = allowedInDb.moderators;
                }
        
                if (!cfg.allowAdminsToConfigure && (!this.commonFunctions.messageSenderIsChatOwner(admins, msg)) && !allowedAdmins.contains(msg.from.id.toString())) {
