...
function containsBlackListedWord(msg, words){
    if(words)
        for(let w of words)
            if(msg.text.toLowerCase().indexOf(w) !== -1) return true // we are keeping words Non Case sensitive
    return false
}
...
module.exports.filterReducer = function filterReducer(msg, cfg, admins, words) {
    let result = false;
    ...
    // Avoids Deletion of admin messages
    if (!messageSenderIsAdmin(admins, msg)) {
        ...
        || cfg.deleteBlacklist && containsBlackListedWord(msg,words) 
